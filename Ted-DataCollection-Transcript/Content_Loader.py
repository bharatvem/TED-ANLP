...
def main(talk_url):
    ...
    try:
        ...
        transcript = soup.find_all('h4', attrs={'class': 'talk-link__speaker'})
        for tag in transcript:
            tdTags = tag.text
            author = tdTags.replace('\n',' ')
        resultlinks.append(author)
...
if __name__ == "__main__":
    ...
    wronglinks = []
    correctlinks = []
    allcontent = pd.DataFrame( columns=['author','topic','poated','link'])
    for i in range(0,count):
        ...
        returnvalues.append(talk_url)
        row = []
        row.append(returnvalues)
        allcontent = allcontent.append(pd.DataFrame(row, columns=['author','topic','poated','link']),ignore_index=True)
    ...
    allcontent.to_csv('allcontent.csv')
